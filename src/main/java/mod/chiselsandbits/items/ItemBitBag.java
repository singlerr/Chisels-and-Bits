			...
			final ItemStack is )
	{
		if ( is != null && is.getItem() instanceof ItemChiseledBit )
		{
			...
			for ( int slot = 0; slot < inv.getSizeInventory(); slot++ )
			{
				...
				if ( which != null && which.getItem() == is.getItem() && ( ItemChiseledBit.sameBit( which, ItemChiseledBit.getStackBitState( is ) ) || is.getItemDamage() == OreDictionary.WILDCARD_VALUE ) )
				{
