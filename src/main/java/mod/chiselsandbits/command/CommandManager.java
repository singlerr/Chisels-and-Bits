...
public class CommandManager
{
    ...
    private int redoFor(final CommandContext<CommandSource> context) throws CommandSyntaxException
    {
        final PlayerEntity target = EntityArgument.getPlayer(context, "target");
        final IChangeTracker tracker = IChangeTrackerManager.getInstance().getChangeTracker(target);
        if (tracker.getChanges().isEmpty())
        {
            context.getSource().sendFailure(new StringTextComponent("No changes available to redo"));
            return 1;
        }
        ...
        {
            context.getSource().sendFailure(new StringTextComponent("Can not redo"));
            return 2;
        }
    }

    private int undoFor(final CommandContext<CommandSource> context) throws CommandSyntaxException
    {
        final PlayerEntity target = EntityArgument.getPlayer(context, "target");
        final IChangeTracker tracker = IChangeTrackerManager.getInstance().getChangeTracker(target);
        if (tracker.getChanges().isEmpty())
        {
            context.getSource().sendFailure(new StringTextComponent("No changes available to undo"));
            return 1;
        }
        ...
        {
            context.getSource().sendFailure(new StringTextComponent("Can not undo"));
            return 2;
        }
    }
}
