...
public class BlockBrokenChange implements IChange
{
    ...
    public BlockBrokenChange(final World world, final PlayerEntity player, final BlockPos blockPos, final IMultiStateSnapshot brokenSnapshot, final boolean bitBased) {
        this.world = world;
        this.player = player;
    ...
    private void insertSnapshotStackOrBits() {
        if (!bitBased) {
            player.inventory.add(brokenSnapshot.toItemStack().toBlockStack());
            return;
        }
