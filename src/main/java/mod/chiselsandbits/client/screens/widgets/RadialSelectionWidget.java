...
public class RadialSelectionWidget extends AbstractChiselsAndBitsWidget
{
    ...
    public void renderButton(final @NotNull MatrixStack matrixStack, final int mouseX, final int mouseY, final float partialTicks)
    {
        ...
        if (isMouseInSection && hoveredItemIndex >= 0 && hoveredItemIndex < modes.size() && modes.get(hoveredItemIndex).isActive())
        {
            if (Configuration.getInstance().getClient().enableMouseIndicatorInRadialMenu.get())
            {
                ...
                drawTorus(matrixStack, startOfMouseArcAngle - 90, mouseArcAngle, innerRadius, outerRadius);
            }
        ...
        modes.forEach(mode -> {
            if (mode.isActive())
            {
                final int modeIndex = modes.indexOf(mode);
                final float itemTargetAngle = ((modeIndex + 0.5f) * itemArcAngle) + sectionStartAngle;
                renderModeIcon(matrixStack, innerRadius, outerRadius, itemTargetAngle, iconScaleFactor, iconTextSpacer, mode, fontRenderer);
            }
        });
        RenderSystem.disableTexture();
      ...
      final FontRenderer fontRenderer)
    {
        ...
        final ITextComponent name = mode.getMultiLineDisplayName();
        final List<IReorderingProcessor> lines = fontRenderer.split(name, 75);
