...
class JSONReader {
    ...
    createDiv(element, large = false, stacksize = 0, shapeless = false) {
        ...
        let recipe = this;
        output += `<div `;
        ((Array.isArray(element) && element.length > 1) ? (large ? output += `class="invslot invslot-large animate"` : output += `class="invslot animate"`) : (large ? output += `class="invslot invslot-large"` : output += `class="invslot"`));
        output += ` >`;
        if (element !== "") {
            if ((Array.isArray(element) && (element.length > 1 || element.length === 0))) {
                element.forEach(item => {
                    output += `
                    <div class="invslot-item switch"
                    `;
                    (item !== element[0] ? output += `style="display: none"` : output += ``);
                    output += `>
                      <img src="../../assets/images/items/` + item + `.png" alt="` + item + `" class="inv-sprite" data-toggle="tooltip" title="` + recipe.createTooltip(item) + `">
                    </div>`;
                });
            } else {
                ...
                output += `
                    <div class="invslot-item">
                      <img src="../../assets/images/items/` + element + `.png" alt="` + element + `" class="inv-sprite" data-toggle="tooltip" title="` + recipe.createTooltip(element) + `">
                    </div>`;
            }
        ...
        <div class="crafting-table">
          <div class="ct-input">
            ...
            <div class="ct-row">`;
                ...
                if (data.recipes[i].isShapeless) {
                    output += `
            <div class="ct-icons">
              <div class="ct-shapeless" title="This recipe is shapeless. The items may be placed anywhere in the crafting grid."></div>
            </div>`;
                }
                output += `<div class="ct-arrow"></div>
          <div class="ct-output">
            `;
                output += recipe.createDiv(data.recipes[i].products, true, data.recipes[i].amount, data.recipes[i].isShapeless);
                output += `
        </div>`;
